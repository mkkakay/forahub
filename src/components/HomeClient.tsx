...
function SdgCard({ sdg, count }: { sdg: number; count: number }) {
  ...
  return (
    <Link
      href={`/events?sdg=${sdg}`}
      className="group bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md hover:scale-105 hover:-translate-y-0.5 transition-all duration-200 overflow-hidden block"
    >
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
      ...
      <div className="px-3 py-2">
        <p className="text-xs font-semibold text-gray-700 leading-tight">{SDG_LABELS[sdg]}</p>
        {count > 0 && (
          <p className="text-xs text-gray-400 mt-0.5">{count} event{count !== 1 ? "s" : ""}</p>
        )}
      </div>
    </Link>
  );
